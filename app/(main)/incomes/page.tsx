...
export default function IncomesPage() {
  ...
  const monthTotal = useMemo(
    () => monthIncomes.reduce((s, t) => s + t.amount, 0),
    [monthIncomes],
  );
  ...
  const categories = useMemo(
    () => [...new Set(monthIncomes.map((t) => t.category))].filter(Boolean).sort(),
    [monthIncomes],
  );
  ...
  }, [monthIncomes, filterCategory, searchQuery, sortOrder]);
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-700/60 border-t border-slate-700/60">
            ...
            <div className="px-4 py-3">
              ...
              <p className="font-mono text-base font-semibold text-slate-200">
                {format(allIncomes.reduce((s, t) => s + t.amount, 0))}
              </p>
              <p className="text-[10px] text-slate-500 mt-0.5">
                {allIncomes.length} total
              </p>
            </div>
          </div>
        </div>
          ...
          <CardContent className="p-0">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm font-medium text-muted-foreground">
                  {hasActiveFilters ? "No income matches your filters" : "No income this month"}
                </p>
                {!hasActiveFilters && (
                  <p className="text-xs text-muted-foreground/60 mt-1">
                    Click &ldquo;Add Income&rdquo; to record one
                  </p>
                )}
              </div>
            ) : (
              <div className="divide-y divide-border">
                {filtered.map((income) => (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full text-xs font-bold flex items-center justify-center shrink-0 ${getCategoryColor(income.category)}`}
                    >
            ...
            {filtered.length > 0 && (
              <div className="px-4 py-3 border-t bg-muted/20 flex items-center justify-between text-xs text-muted-foreground">
                <span>{filtered.length} income{filtered.length !== 1 ? "s" : ""}</span>
