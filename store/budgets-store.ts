...
interface BudgetsState {
  ...
  deleteAllBudgets: (month?: number, year?: number) => Promise<{ success: boolean; deletedBudgets: number; deletedTransactions: number } | void>;
}
...
export const useBudgetsStore = create<BudgetsState>((set, get) => ({
  ...
  deleteAllBudgets: async (month?: number, year?: number) => {
    set({ loading: true, error: null });
    try {
      const state = get();
      const targetMonth = month ?? state.currentMonth;
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to delete all budgets");
      }
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to delete all budgets";
